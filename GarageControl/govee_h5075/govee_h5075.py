    ...
    async def scan(consumer, duration: int = 20, unique: bool = True, mac_filter: str = None, progress=None):
        ...
        def callback(device: BLEDevice, advertising_data: AdvertisementData):
            ...
            if unique is False or device.address not in found_devices:
                ...
                if device.name and device.address.upper().startswith(GoveeThermometerHygrometer.MAC_PREFIX) or \
                   (mac_filter and device.address.upper() == mac_filter.upper()) or \
                   (not mac_filter and device.name and (device.name.startswith("Govee_H5075") or device.name.startswith("GVH5075"))):

                    LOGGER.debug(
                        f"Found {device.name if device.name else ''} {device.address} {advertising_data.rssi}dBm")
